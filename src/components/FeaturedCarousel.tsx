...
const FeaturedCarousel: React.FC<FeaturedCarouselProps> = ({ activities }) => {
  ...
  return (
    <div className="relative w-full h-[450px] overflow-hidden rounded-2xl shadow-xl">
      ...
      {/* Navigation Arrows */}
      <button
        className="absolute top-1/2 left-4 -translate-y-1/2 p-3 bg-white/80 rounded-full shadow-md hover:bg-white transition-colors"
      ...
      <button
        className="absolute top-1/2 right-4 -translate-y-1/2 p-3 bg-white/80 rounded-full shadow-md hover:bg-white transition-colors"
        onClick={goToNext}
